...
// ---------------------------------------------------------------- edge functions
{
  ...
  const { data: d4, error: e4 } = await anon.functions.invoke('signup-clinic', { body: { clinic_name: 'x', owner_name: 'x', email: 'nobody@example.com', password: 'password123', accept_terms: true } })
  ok('signup-clinic refuses unverified signups', (e4?.context?.status === 401) || /verify/i.test(d4?.error || ''))
  const { data: d5, error: e5 } = await anon.functions.invoke('signup-clinic', { body: { clinic_name: 'x', owner_name: 'x', email: 'nobody@example.com', password: 'password123' } })
  ok('signup-clinic requires Terms/DPA acceptance', (e5?.context?.status === 400) || /accept/i.test(d5?.error || ''))
  // GDPR controls
  const { data: pat } = await staff.from('dental_patients').select('id').eq('clinic_id', HARBOUR).limit(1).single()
  const { error: logErr } = await staff.from('dental_access_log').insert({ clinic_id: HARBOUR, patient_id: pat.id, user_email: 'test', action: 'view' })
  ok('staff can write to the access log', !logErr, logErr?.message)
  const { data: logRows } = await dentist.from('dental_access_log').select('id').limit(1)
  ok('non-admin cannot read the access log', (logRows || []).length === 0)
  const { error: anonErr } = await dentist.rpc('dental_anonymise_patient', { p_patient_id: pat.id })
  ok('non-admin cannot anonymise a patient', !!anonErr, anonErr?.message)
}
